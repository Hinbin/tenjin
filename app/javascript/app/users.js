$(document).on("turbolinks:load", () => {
  if ($("#students-table").length) {
    ...
    $(document).on("ajax:success", (event) => {
      ...
      const table = $(`tr[data-id="${response.id}"] .reset-password`)
        .closest("table")
        .DataTable();
      const oldCell = $(`tr[data-id="${response.id}"] .reset-password`).parent(
        "td",
      );
      table
        .cell(oldCell)
        .data(`<div class="new-password">${response.password}</div>`)
        .draw();
    });
  }
  ...
  if (page.controller() === "users" || page.controller() === "schools") {
    ...
    if (!$.fn.dataTable.isDataTable("#students-table")) {
      $("#students-table").DataTable({
        paging: true,
        dom:
          "<'row'<'col-sm-12 col-md-4 pt-4'B><'col-sm-12 col-md-4'l><'col-sm-12 col-md-4'f>>" +
          "<'row'<'col-sm-12'tr>>" +
