...
module Theme
  ...
  module Resolver
    ...
    def css_vars(skin: SkinCatalog::DEFAULT_SKIN, palette: SkinCatalog::DEFAULT_PALETTE,
                 dark: SkinCatalog::DEFAULT_DARK, glow: DEFAULT_GLOW)
      meta = SkinCatalog.meta(skin)
      pal  = SkinCatalog.palette(skin, palette)
    ...
    def style_string(**opts)
      css_vars(**opts).map { |k, v| "#{k}:#{v}" }.join(';')
    end
  end
end
