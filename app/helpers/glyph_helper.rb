...
module GlyphHelper
  # name => inner SVG markup. `%{c}` is the colour for solid inner dots; the outer stroke is set
  ...
  GLYPHS = {
    ...
    target:  '<circle cx="12" cy="12" r="8.5"/><circle cx="12" cy="12" r="4.5"/><circle cx="12" cy="12" r="0.8" fill="%{c}"/>',
    ...
    dice:    '<rect x="4" y="4" width="16" height="16" rx="3"/><circle cx="9" cy="9" r="1.2" fill="%{c}" stroke="none"/><circle cx="15" cy="15" r="1.2" fill="%{c}" stroke="none"/><circle cx="12" cy="12" r="1.2" fill="%{c}" stroke="none"/>',
    tag:     '<path d="M4 4h7l9 9-7 7-9-9z"/><circle cx="8.5" cy="8.5" r="1.4" fill="%{c}" stroke="none"/>',
