...
RSpec.describe Challenge::UpdateChallengeProgress do
  ...
  context 'when updating a number correct challenge' do
    ...
    let(:quiz_full_marks) { create(:quiz, subject: subject, num_questions_asked: 10, answered_correct: 10, active: false) }
    let(:quiz_7_out_of_10) { create(:quiz, subject: subject, num_questions_asked: 10, answered_correct: 7, active: false) }
    let(:quiz_1_out_of_3) { create(:quiz, subject: subject, num_questions_asked: 3, answered_correct: 1, active: false) }
    ...
    it 'flags challenge as complete if the required number of questions have been asnwered correctly' do
  ...
  context 'when updating a 5 streak challenge' do
    let(:quiz_streak_of_five) { create(:quiz, subject: subject, streak: 5) }
    let(:quiz_streak_of_three) { create(:quiz, subject: subject, streak: 3) }
  ...
  context 'when updating a 50 point challenge' do
    ...
    let(:quiz_five_points) { create(:quiz, subject: subject, user: student) }
    ...
    it 'increases the progress by the correct amount' do
      described_class.new(quiz_five_points, 'number_of_points', 2).call
      expect(ChallengeProgress.first.progress).to eq(47)
    end

    it 'flags challenge as complete when a streak of 5 is obtained' do
      described_class.new(quiz_five_points, 'number_of_points', 5).call
      expect(ChallengeProgress.first.completed).to eq(true)
    end
  end
end
