...
RSpec.describe Question::ImportQuestions, '#call', default_creates: true do
  context 'when importing questions' do
    ...
    it 'validates questions successfully' do
      result = described_class.new(JSON.generate(multiple_lessons), topic).call
      expect(result.success?).to eq(true)
    end

    it 'imports boolean questions' do
      result = described_class.new(JSON.generate([build(:question_import_hash_boolean)]), topic).call
      expect(result.success?).to eq(true)
    end
    ...
    it 'adds questions with no lesson information' do
      result = described_class.new(JSON.generate(no_lessons), topic).call
      expect(result.success?).to eq(true)
    end
  end
end
