...
RSpec.describe 'Super views all statistics', type: :system, js: true, default_creates: true do
  ...
  context 'when looking at questions asked' do
    let(:new_stat) { create(:user_statistic, user: student, week_beginning: DateTime.now.beginning_of_week) }
    let(:old_stat) { create(:user_statistic, user: create(:student, school: school), week_beginning: two_weeks_ago) }
    let(:new_stat_different_school) { create(:user_statistic, week_beginning: DateTime.now.beginning_of_week) }
  ...
  context 'when looking at homeworks completed' do
    let(:homework_progress) { create(:homework_progress, user: student, completed: true, updated_at: DateTime.now.beginning_of_week) }
    let(:old_homework_progress) { create(:homework_progress, user: student, completed: true, updated_at: two_weeks_ago) }
    let(:homework_progress_different_school) { create(:homework_progress, completed: true, updated_at: DateTime.now.beginning_of_week) }
