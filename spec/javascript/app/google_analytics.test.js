...
// Import the module — this registers the turbo:load listener.
import "../../../app/javascript/lib/google_analytics";

describe("google_analytics turbo:load handler", () => {
  let mockGtag;
  ...
  function fireTurboLoad(url = "https://example.com/page") {
    const event = new CustomEvent("turbo:load", { detail: { url } });
    document.dispatchEvent(event);
  }

  it("calls gtag with the GA ID read from the meta tag", () => {
    fireTurboLoad("https://example.com/about");
  ...
  it("does not throw when gtag is not defined on window", () => {
    delete global.gtag;
    expect(() => fireTurboLoad()).not.toThrow();
  });
});
